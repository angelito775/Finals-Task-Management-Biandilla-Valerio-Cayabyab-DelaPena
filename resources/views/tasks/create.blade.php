    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div class="p-8 border-b border-slate-100">
                <h2 class="text-xl font-semibold text-slate-900">Create New Task</h2>
                <p class="text-slate-500 text-sm mt-1">Add a new task to your workspace.</p>
            </div>
            ...
            <form action="{{ route('tasks.store') }}" method="POST" class="p-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        ...
                        <input id="title" name="title" value="{{ old('title') }}" placeholder="What needs to be done?" class="w-full px-3 py-2.5 rounded-lg border {{ $errors->has('title') ? 'border-red-300 focus:ring-red-500' : 'border-slate-300 focus:ring-indigo-500' }} focus:ring-2 focus:border-transparent outline-none transition-shadow" />
                        @error('title')
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="description" name="description" rows="3" placeholder="Add any extra details or notes here..." class="w-full px-3 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition-shadow resize-none">{{ old('description') }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <select id="priority_id" name="priority_id" class="w-full px-3 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition-shadow bg-white">
                            @foreach($priorities as $priority)
                                <option value="{{ $priority->id }}" {{ old('priority_id', 2) == $priority->id ? 'selected' : '' }}>{{ $priority->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input id="due_date" type="date" name="due_date" value="{{ old('due_date') }}" min="{{ now()->toDateString() }}" class="w-full px-3 py-2.5 rounded-lg border {{ $errors->has('due_date') ? 'border-red-300 focus:ring-red-500' : 'border-slate-300 focus:ring-indigo-500' }} focus:ring-2 focus:border-transparent outline-none transition-shadow" />
                        @error('due_date')
                            <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="mt-8 pt-6 border-t border-slate-100 flex items-center justify-end gap-3">
                    ...
                    <button type="submit" class="btn-primary min-w-[140px]">Save Task</button>
                </div>
            </form>
        </div>
    </div>
